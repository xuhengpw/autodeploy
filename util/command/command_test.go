...
func TestCmdRun(t *testing.T) {
	c := &Command{
		//Cmd: "ssh -o StrictHostKeyChecking=no  -p 22 exapp@47.97.46.201 'cd ~/services/BankCard-Provider/script/bin" +
		//	" && ./BankCard.sh start && sed '/Stopped/q' <(tail -f ~/services/BankCard-Provider/logs/wrapper.log) '",

		Cmd:     "ssh -o StrictHostKeyChecking=no  -p 22 exapp@47.97.46.201 'sed '/Stopped/q' <(tail -f ~/services/BankCard-Provider/logs/wrapper.log) '",
