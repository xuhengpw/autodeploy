...
//将hookbuild执行的结果添加到已执行完毕的build的返回结果中（output）
func (b *Build) FinishHookBuild(result []*command.TaskResult) error {
	...
	if ok := build.UpdateByFields(updateData, model.QueryParam{
		...
		return errors.New("update deploy build output failed")
	}
